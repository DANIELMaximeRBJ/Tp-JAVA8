...
public class Stream_01_Test {
    ...
    public void test_stream_filter_and_match() throws Exception {
        ...
        List<Order> result = orders.stream().filter(commande -> commande.getCustomer().getFirstname().equals("Johnny") && commande.getPizzas().stream().anyMatch(pi -> pi.getPrice() >= 1300)).collect(Collectors.toList());
    ...
    public void test_stream_findFirst() throws Exception {
        ...
        Optional<Order> result = orders.stream().filter(commande -> commande.getCustomer().getFirstname().equals("Sophie")).findFirst();
